...
protected:
	...
	virtual int writeSCS2(unsigned char *nDat, int nLen);
	virtual int readSCS(unsigned char *nDat, int nLen,uint8_t id);//input nLen byte
	virtual int writeSCS(unsigned char bDat);//output 1 byte
	virtual int writeSCS2(unsigned char bDat);//output 1 byte
	virtual void rFlushSCS();//
	virtual void wFlushSCS();//
public:
	unsigned long int IOTimeOut;//I/O timeout
	HardwareSerial *pSerial;//serial pointer
	HardwareSerial *pSerial2;//serial pointer
	int Err;
public:
