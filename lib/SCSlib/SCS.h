...
protected:
	...
	virtual int readSCS(unsigned char *nDat, int nLen,uint8_t id) = 0;
	virtual int writeSCS(unsigned char bDat) = 0;
	virtual int writeSCS2(unsigned char *nDat, int nLen) = 0;
	virtual int writeSCS2(unsigned char bDat) = 0;
...
protected:
	...
	int checkHead(uint8_t ID); // Frame header detection
};
#endif
