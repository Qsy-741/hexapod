...
#define LEG_LEN1 57.f  // 腿部第一连杆长度（单位mm）
#define LEG_LEN2 82.f  // 腿部第二连杆长度（单位mm）
#define LEG_LEN3 142.f // 腿部第三连杆长度（单位mm）

#define CHASSIS_LEN 208.f        // 底盘长度（y轴方向）
#define CHASSIS_WIDTH 170.f      // 底盘宽度（x轴方向）
#define CHASSIS_FRONT_WIDTH 128.f // 底盘前端宽度（x轴方向）

#define N_POINTS 10                  // 点的数量（必须是偶数）16
#define THETA_STAND_2 20.0f / 180.0f * PI // 机械腿站立时最后两个关节的角度
